...
class IMDbBot:
    ...
    def import_review(self, imdb_id, rating, review):
        ...
        try:
            self.wait(10)
            stars_parent_element = self.driver.find_element(
                By.CLASS_NAME, IMDB_STARS_ELEMENTS_CLASS_NAME
            )
            stars = stars_parent_element.find_elements(By.XPATH, "./*")
        except Exception as e:
            exception_handler(
                "Exception thrown when finding stars element: {}".format(e)
            )
        try:
            self.driver.execute_script("arguments[0].click();", stars[imdb_rating - 1])
        except ValueError as e:
            exception_handler(
                "ValueError thrown when trying to click stars: {}".format(e)
            )
        ...
        try:
            ...
            self.driver.find_element(By.XPATH, IMDB_HEADLINE_ELEMENT_XPATH).send_keys(
                title
            )
        except Exception as e:
            exception_handler(
                "Exception thrown when accessing headline element: {}".format(e)
            )
        ...
        try:
            ...
            self.driver.find_element(By.XPATH, IMDB_REVIEW_ELEMENT_XPATH).send_keys(
                review
            )
        except Exception as e:
            exception_handler(
                "Exception thrown when accessing review element: {}".format(e)
            )
        ...
        try:
            ...
            exception_handler(
                "Exception thrown when clicking spoiler element: {}".format(e)
            )
        ...
        try:
            ...
            exception_handler(
                "Exception thrown when clicking submit element: {}".format(e)
            )
        ...
        try:
            ...
            exception_handler(
                "Exception thrown when getting imdb movie page: {}".format(e)
            )
    ...
    def add_to_cinema_personified_list(self, imdb_id, title, year):
        ...
        try:
            ...
            exception_handler(
                "Exception thrown when getting cinema personified list: {}".format(e)
            )
        ...
        try:
            ...
            self.driver.find_element(
                By.XPATH, IMDB_ADD_TO_LIST_SEARCH_ELEMENT_XPATH
            ).send_keys("{} {} ({})".format(imdb_id, title, year))
        except Exception as e:
            exception_handler(
                "Exception thrown when sending keys to add to list search: {}".format(e)
            )
        ...
        try:
            self.wait(10)
            self.driver.find_element(
                By.XPATH, IMDB_FIRST_MOVIE_IN_SEARCH_ELEMENT_XPATH
            ).click()
        except Exception as e:
            exception_handler(
                "Exception thrown when clicking movie in list search: {}".format(e)
            )
        ...
        try:
            ...
            exception_handler(
                "Exception thrown when saving movie in Cinema Personified list: {}".format(
                    e
                )
            )
...
def rate_on_imdb(imdb_id, rating):
    ...
    req_body = {
        ...
        "variables": {"rating": rating, "titleId": imdb_id},
    }
    headers = {"content-type": "application/json", "cookie": imdb_cookie}

    resp = requests.post(
        "https://api.graphql.imdb.com/", json=req_body, headers=headers
    )
