...
def write_proto(proto):
    ...
    if not os.path.exists(filename):
        ...
        print(
            "File already exists! Either choose different file or move current to redux."
        )
...
def create_proto(redux=False):
    ...
    review = movie_pb2.Movie.Review(
        ...
        soundtrack=get_generic(category="Soundtrack"),  # "Soundtrack ()",
        cinematography=get_field(
            infobox, "Cinematography"
        ),  # parse_cinematography(infobox),
...
def get_field(infobox, key):
    ...
    if key == "Directed by":
        ...
        direction.comments = "macroscale is; microscale is; storytelling is; tension is"
        return direction
    elif key == "Music by":
...
def get_acting(infobox):
    ...
    for name in infobox["Starring"]:
        performance = movie_pb2.Movie.Review.Acting.Performance(
            actor=movie_pb2.Movie.Review.Person(name=name)
        )
...
def get_writing(infobox):
    ...
    if story != None and screenplay != None:
        story.rating = "TODO"
        story.comments = "The concept is; the plot structure is; character writing is"
        screenplay.rating = "TODO"
        screenplay.comments = (
            "The dialogue is; the humor is; the symbolism is; the foreshadowing is"
        )
...
def find_release_year(infobox):
    ...
    return (
        int(release_year.split("\xa0")[2])
        if "\xa0" in release_year
        else int(release_year.partition(", ")[-1].partition(" ")[0])
    )
...
def sanity_check(proto):
    ...
    for performance in proto.review.acting.performance:
        if performance.rating == "TODO":
            raise ValueError(
                "{} needs to be given a tier".format(performance.actor.name)
            )
